...
def record(Motion_Detected, frame, loop_time, videoName, cap):
	frame_rec_count = 0
	fourcc = cv2.VideoWriter_fourcc(*'XVID')
	out = cv2.VideoWriter(videoName,fourcc, 30.0, (960, 540))
	if(Motion_Detected==True):
		try:
			while(frame_rec_count<240):
				...
				if(cv2.waitKey(1) & 0xFF == ord('q')):
					print("Recording Process stopped sat:", loop_time)
					return 1
					break
				else:
					continue
...
while(True):
	...
	if(loop_time%10<=0.04):	# 0.04 is roughly the first recorded time interval after each new second
		delay = False	# A remainder of 0 too precise for how clock works
		delay_time = loop_time + 10
	else:
		delay = True
	loop_time_text = str(loop_time) # displays the current time (as camara activated)
	ret, frame = cap.read()
	#[line below] -> add date/timestamp to footage & name to window
	...
	if(loop_time <= 1):
		...
		if((result > 0.45) and (delay==False)):	
			print("Motion detected!", loop_time)
			detected_motion = True
			try:
				footage = record(detected_motion, frame, loop_time, videoName, cap)
				continue
			except:
